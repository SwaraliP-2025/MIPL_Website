...
const AdminLogin = () => {
  const [apiUrl, setApiUrl] = useState(localStorage.getItem('cms_api_url') || '');
  ...
  const testConnection = async () => {
    if (!apiUrl.trim()) {
      toast({ title: "Error", description: "Please enter the API URL first.", variant: "destructive" });
      return;
    }
    setPingStatus('testing');
    try {
      const res = await fetch(`${apiUrl.trim()}?action=ping`);
      const data = await res.json();
      if (data.success) {
        setPingStatus('ok');
        localStorage.setItem('cms_api_url', apiUrl.trim());
  ...
  const handleSubmit = async (e) => {
    e.preventDefault();
    if (!apiUrl.trim()) {
      toast({ title: "Error", description: "Please enter the API URL.", variant: "destructive" });
      return;
    }
    setIsLoading(true);
    localStorage.setItem('cms_api_url', apiUrl.trim());
    ...
    try {
      ...
      if (email === 'info@consultmipl.com' && password === 'Mipl@2000') {
        ...
        toast({ title: "Login Failed", description: "Could not authenticate. Check credentials and API URL.", variant: "destructive" });
      }
    } finally {
      setIsLoading(false);
    }
